...
When you provide one or more of these options as command line parameters, it overrides the default configuration or any corresponding profile setting for that single command\. Each option takes a string argument with a space or equals sign \(=\) separating the argument from the option name\. If the argument value contains a space, you must use quotation marks around the argument\.
